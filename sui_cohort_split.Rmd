...
```{r}
ideate_y_glm <-
  svyglm(ideate~SEX+io16+de34+marital_status+de20+emp2+financial_status+SN7,
...
```{r}
ideate_y_glm <-
  svyglm(ideate~SEX+io16+marital_status+financial_status+Malad+Maltreat+aff+ied+any_cc,
         family = quasibinomial("logit"), design = youngest_design)
ideate_m_glm <-
  svyglm(ideate~SEX+marital_status+ChdIll+aff+ied,
         family = quasibinomial("logit"), design = middle_design)
ideate_e_glm <-
  svyglm(ideate~SEX+marital_status+financial_status+Malad+Maltreat+aff,
         family = quasibinomial("logit"), design = eldest_design)
summary(ideate_y_glm)
summary(ideate_m_glm)
summary(ideate_e_glm)
...
```{r}
export_summs(ideate_y_glm, ideate_m_glm, ideate_e_glm,
             error_format = "[{conf.low}, {conf.high}]",
             exp = TRUE, 
             model.names = c("aOR Youngest Cohort", 
                             "aOR Middle Cohort",
                             "aOR Eldest Cohort"),
             coefs = c("Sex: Male" = "SEX1",
                       ...
                       "Inadequate Finances" = "financial_statusinadequate",
                       "Adequate Finances" = "financial_statusadequate",
             ...
             statistics = c(N = "nobs"),
             to.file = "docx",
             file.name = "age_model.docx")
```

# Frequency Tables - Youngest

```{r}
svytable(~SEX, design = youngest_design) %>% 
  addmargins() %>%
  round()
```

```{r}
svytable(~io16, design = youngest_design) %>% 
  addmargins() %>%
  round()
```

```{r}
svytable(~marital_status, design = youngest_design) %>% 
  addmargins() %>%
  round()
```

```{r}
svytable(~financial_status, design = youngest_design) %>% 
  addmargins() %>%
  round()
```

```{r}
svytable(~Malad, design = youngest_design) %>% 
  addmargins() %>%
  round()
```

```{r}
svytable(~Maltreat, design = youngest_design) %>% 
  addmargins() %>%
  round()
```

```{r}
svytable(~aff, design = youngest_design) %>% 
  addmargins() %>%
  round()
```

```{r}
svytable(~ied, design = youngest_design) %>% 
  addmargins() %>%
  round()
```

```{r}
svytable(~any_cc, design = youngest_design) %>% 
  addmargins() %>%
  round()
```

# Frequency Tables - Middle

```{r}
svytable(~SEX, design = middle_design) %>% 
  addmargins() %>%
  round()
```

```{r}
svytable(~marital_status, design = middle_design) %>% 
  addmargins() %>%
  round()
```

```{r}
svytable(~ChdIll, design = middle_design) %>% 
  addmargins() %>%
  round()
```

```{r}
svytable(~aff, design = middle_design) %>% 
  addmargins() %>%
  round()
```

```{r}
svytable(~ied, design = middle_design) %>% 
  addmargins() %>%
  round()
```

# Frequency Tables - Eldest

```{r}
svytable(~SEX, design = eldest_design) %>% 
  addmargins() %>%
  round()
```

```{r}
svytable(~marital_status, design = eldest_design) %>% 
  addmargins() %>%
  round()
```

```{r}
svytable(~financial_status, design = eldest_design) %>% 
  addmargins() %>%
  round()
```

```{r}
svytable(~Malad, design = eldest_design) %>% 
  addmargins() %>%
  round()
```

```{r}
svytable(~Maltreat, design = eldest_design) %>% 
  addmargins() %>%
  round()
```

```{r}
svytable(~aff, design = eldest_design) %>% 
  addmargins() %>%
  round()
```
